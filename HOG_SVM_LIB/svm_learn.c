...
void svm_learn_classification_details(DOC **docs, double *_class, long int
  ...
  if(verbosity>=1) {
	...
	*res_sv_num = _sv_num;
	...
	model_length+=a[i]*label[i]*lin[i];
	  ...
	  _vcdim = estimate_margin_vcdim(model, model_length, example_length, kernel_parm);
	  *res_vcdim = _vcdim;
	...
	double *alpha)
{
	long res_sv_num;
	long res_vcdim;
	svm_learn_classification_details(docs, _class, totdoc, totwords, learn_parm, kernel_parm, kernel_cache, model, alpha, &res_sv_num, &res_vcdim);
}
...
{                         /* length of the longest support vector. This is */
  ...
  _WORD nullword;          /* origin of the space. */
