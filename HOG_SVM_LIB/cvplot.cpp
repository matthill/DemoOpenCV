...
void PlotManager::Plot(const std::string figure_name, const std::vector<float> &p, int count, int step,
	int R, int G, int B)
{
	float* fp = new float[p.size()];
	for (size_t i = 0; i < p.size(); i++)
	{
		fp[i] = p[i];
	}
	pm.Plot(figure_name, fp, count, step, R, G, B);
	delete[] fp;
}
...
void plot(const std::string figure_name, const std::vector<float> &p, int count, int step,
	int R, int G, int B)
{
	if (step <= 0)
		step = 1;	
