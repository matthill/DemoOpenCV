...
SVECTOR *create_svector(_WORD *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (_WORD *)my_malloc(sizeof(_WORD)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
	...
	SVECTOR is used */
{
    ...
    sum=(_WORD *)my_malloc(sizeof(_WORD)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
void write_linear_model(char *modelfile, MODEL *model)
{
	FILE *modelfl;
	long j, i, sv_num;
	SVECTOR *v;
	...
	if (verbosity >= 1) {
		printf("Writing linear model file..."); fflush(stdout);
	}
	...
	for (i = 0; i < model->totwords; i++) {
		fprintf(modelfl, "%ld:%.32g ", i+1, model->lin_weights[i + 1]);
	}
	fprintf(modelfl, "\n");
	fclose(modelfl);
	if (verbosity >= 1) {
...
MODEL *read_model(char *modelfile)
{
  ...
  words = (_WORD *)my_malloc(sizeof(_WORD)*(max_words+10));
  ...
  if (model->kernel_parm.kernel_type == LINEAR){
	  ...
	  parse_linear_weight(line, model->lin_weights, model->totwords);
...
void read_documentsBaseMap(char *docfile, DOC ***docs, double **label,
	long int *totwords, long int *totdoc, const int *map, long lengthMap, double classLabel )
{
	char *line, *comment;
	_WORD *words;
	long dnum = 0, wpos, dpos = 0, dneg = 0, dunlab = 0, queryid, slackid, max_docs;
	...
	// long numOfZeros = 0;
	// for (long i = 0; i < lengthMap; i++)
	// {
		// if (map[i] == 0) numOfZeros++;
	// }
	// max_words_doc -= numOfZeros;
	// //ll -= numOfZeros;
	// max_docs -= numOfZeros;
	...
	while ((!feof(docfl)) && fgets(line, (int)ll, docfl)) {
		...
		count++;
		if (lengthMap <= count) break;
		if (map[count] == 0) 
			continue;

		if (doc_label == classLabel)
			doc_label = 1.0;
		else
			doc_label = -1.0;
		(*label)[dnum] = doc_label;
...
int parse_linear_weight(char *line, double *lin_weights, long length){
	register long pos, wpos;
	...
	while (((numread = sscanf(line + pos, "%s", featurepair)) != EOF) &&
		...
		if (sscanf(featurepair, "%ld:%lf%s", &wnum, &weight, junk) == 2) {
			...
			if (wpos > 1 && prewnum >= wnum){
				perror("Features must be in increasing order!!!\n");
				printf("LINE: %s\n", line);
				exit(1);
			}
			prewnum = wnum;
			lin_weights[wpos] = weight;
			wpos++;
		}
	}
	if (wpos > length + 1){
		perror("Features length must be equal length!!!\n");
		exit(1);
	}
	return 1;
}
int parse_document(char *line, _WORD *words, double *label,
